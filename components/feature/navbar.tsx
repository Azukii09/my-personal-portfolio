        ...
        <nav className="fixed px-6 sm:px-8 md:px-16 lg:px-20 w-full">
            <div className={"top-0 flex justify-between h-20"}>
                ...
                <div className={"flex items-center "}>
                            ...
                            {dataNavbar.map((item, index) => (
                                <Link key={index} href={item.link}>
                                    <li
                                        className={
                                        pathName === item.link ?
                                            "bg-primary font-semibold tracking-wide text-contras py-2 lg:px-4 lg:text-xl text-center group relative min-w-20 lg:min-w-32 cursor-pointer overflow-hidden" :
                                            "font-semibold tracking-wide group relative min-w-20 lg:min-w-32 cursor-pointer overflow-hidden py-2 px-4 lg:text-xl text-center transition-colors duration-700 hover:text-contras"}
                                    >
                                        {item.name}
                                        <span
                                            aria-hidden="true"
                    ...
                    <button
                        className={active? "block absolute md:hidden hamburger-active" :"block absolute md:hidden"}
                        id="hamburger"
                        name="hamburger"
                        type="button"
